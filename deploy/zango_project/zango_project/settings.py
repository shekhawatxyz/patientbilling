...
# PLATFORM_ADMIN_ALLOWED_IPS is required for ENV=prod/staging and should contain the deploying admin's IP or CIDR.
_environment = os.environ.get("ENV", "dev").lower()
INTERNAL_IPS = [
    address.strip()
    for address in os.environ.get("PLATFORM_ADMIN_ALLOWED_IPS", "").split(",")
    if address.strip()
]
if _environment in {"prod", "staging"} and not INTERNAL_IPS:
    raise RuntimeError(
        "PLATFORM_ADMIN_ALLOWED_IPS is required when ENV is prod or staging"
    )
